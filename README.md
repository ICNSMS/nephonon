...
**nephonon** is a Fortran-based software package designed for calculating phonon properties and neutron scattering intensities. It supports force constants generated either from Phonopy (via `FORCE_CONSTANTS` file) or directly from Neuroevolution Potentials (NEPs) using an integrated interface.
...
$$
g(\omega) = \frac{1}{N_q} \sum_{\mathbf{q},j} \frac{1}{\sqrt{\pi}\sigma} \exp\left[-\left(\frac{\omega - \omega_{\mathbf{q}j}}{\sigma}\right)^2\right]
$$
...
> [NEPHONON: An Efficient Phonon Calculator Based on Neuroevolution Potentials, Peng-Fei Liu, Jianbo Zhu, Xi Chen, Jingyu Li, Yongsheng Zhang*, and Junrong Zhang*, Comput. Phys. Commun.]

*(Note: Please update this section with the actual publication or repository link once available.)*
